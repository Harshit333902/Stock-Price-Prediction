...
def main():
    file_paths = {
        1: 'C:\\Users\\harsh\\OneDrive\\Desktop\\STOCK PRICE PREDICTION\\Latest Datasets\\Quote-Equity-ADANIPORTS-EQ-06-09-2023-to-06-09-2024.csv',
        2: 'C:\\Users\\harsh\\OneDrive\\Desktop\\STOCK PRICE PREDICTION\\Latest Datasets\\Quote-Equity-TATAGLOBAL-EQ-06-09-2023-to-06-09-2024.csv',
        3: 'C:\\Users\\harsh\\OneDrive\\Desktop\\STOCK PRICE PREDICTION\\Latest Datasets\\Quote-Equity-ZOMATO-EQ-06-09-2023-to-06-09-2024.csv'
    }
    ...
    while True:
        ...
        if choice == 4:
            ...
            X_test_non_lstm = np.arange(len(train_data), len(train_data) + len(test_data)).reshape(-1, 1)
            ...
            non_lstm_test_size = len(X_test_non_lstm)
            lstm_combined_test_size = len(lstm_y_pred)
            min_test_size = min(non_lstm_test_size, lstm_combined_test_size)

            rf_preds = non_lstm_predictions["Random Forest"][:min_test_size]
            dt_preds = non_lstm_predictions["Decision Tree"][:min_test_size]
            gb_preds = non_lstm_predictions["Gradient Boosting"][:min_test_size]
            lstm_preds = lstm_y_pred[:min_test_size]

            combined_predictions = np.mean(
                [rf_preds, dt_preds, gb_preds, lstm_preds.flatten()],
                axis=0
            )

            # Plot the results
            plot_results(
                df.index[-min_test_size:],  # Use the last `min_test_size` dates for the test range
                lstm_y_test[:min_test_size],
            ...
            plt.plot(future_dates, future_predictions, label='Future Predictions (Combined)', color='red', linestyle='dashed')
